...
class autoroute {
	 ...
	 * replacing all \\\\ with / to add support for all platforms. The promise this function 
	 * returns will call the "file" event when the file has been loaded (with the file as parameter) and the "request" event 
	 * when the favicon has been requested (with the request and response as 
	 * parameters).
	 * @example
